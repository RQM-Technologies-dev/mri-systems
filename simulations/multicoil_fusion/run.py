...
def main() -> None:
    ...
    phase_jitter = float(config.get("phase_jitter", 0.8))
    ...
    coil_offsets = rng.uniform(-phase_jitter, phase_jitter, size=(coils, 1, 1))
    coil_images = coil_images * np.exp(1j * coil_offsets)
    ...
    metrics = {
        "scenario_name": "multicoil_fusion",
        ...
        "notes": (
            ...
            "Phase-disagreement stress test for baseline-vs-candidate behavior."
        ),
        "baseline": {
    ...
    (output_dir / "summary.md").write_text(
        "# Multicoil fusion stress-test summary\n\n"
        "## Scenario\n"
        "- Name: `multicoil_fusion`\n"
        "- Synthetic only: `true`\n"
        "- Description: synthetic coil-disagreement stress test.\n\n"
        "## Input configuration\n"
        f"- seed: `{seed}`\n"
        f"- size: `{size}`\n"
        f"- coils: `{coils}`\n"
        f"- phase_jitter: `{phase_jitter}`\n\n"
        "## Methods\n"
